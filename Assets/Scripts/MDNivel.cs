...
public class MDNivel : MonoBehaviour
{
    ...
    void Start()
    {
        tentativas = 4;
        fases = 1;
        nAtual = PlayerPrefs.GetInt("nAtual");
    ...
    {
        int valorAtual = PlayerPrefs.GetInt("nAtual", 0);
        PlayerPrefs.SetInt("bloqueado" + (valorAtual + 1), 0);
        valorAtual += 1;
        PlayerPrefs.SetInt("nAtual", valorAtual);
        PlayerPrefs.Save();
        Debug.Log("Valor atual de nAtual: " + valorAtual);
        SceneManager.LoadScene("MatDourado2");
        // Chamar AtivarBotaoNivel através de cenaNivel
    }
